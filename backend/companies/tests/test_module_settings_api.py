...
from companies.models import DEFAULT_ROLE_DISABLED_MODULES, Company
...
class CompanyModuleSettingsApiTest(TestCase):
        ...
        self.assertEqual(
            response.json(),
            {
                ...
                "role_disabled_modules": {
                    role: list(modules) for role, modules in DEFAULT_ROLE_DISABLED_MODULES.items()
                },
            },
        )
    ...
    def test_role_disabled_modules_persist_and_reject_unknown_role(self) -> None:
        response = self.http.patch(
            "/api/v1/company/module-settings/?company=sportmax",
            data={"role_disabled_modules": {"reception": ["marketing", "sales", "marketing"]}},
            content_type="application/json",
            **self.auth_headers(),
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.json()["role_disabled_modules"],
            {
                "admin": [],
                "manager": [],
                "reception": ["marketing", "sales"],
                "user": list(DEFAULT_ROLE_DISABLED_MODULES["user"]),
            },
        )
    ...
    def test_owner_role_always_sees_full_menu(self) -> None:
        ...
        self.company.role_disabled_modules = {"owner": ["sales"], "reception": ["sales"]}
        ...
        self.assertEqual(self.company.effective_disabled_modules("reception"), ["marketing", "sales"])
