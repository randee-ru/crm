...
- После обновления кода на сервере нужно делать пересборку:
  - `docker compose -f docker-compose.prod.yml --env-file .env.prod up -d --build backend frontend telephony-sync`
...
- Добавлены менеджеры:
  - Александра Дмитриевна Цыганкова (`lexiko@internet.ru`) - Mango `11`
  - Оксана Владимировна Смелкова (`oksanasmelkova702@gmail.com`) - Mango `10`
- Добавлены и задеплоены Telegram-уведомления владельцу (см. раздел «Telegram-уведомления» ниже)
- Исправлен 500 на `/api/v1/telephony/calls/<id>/stream/`, когда в `CallLog.recording_file` оставалась ссылка на удалённый файл:
  - теперь локальная запись проверяется через storage перед открытием
  - при отсутствии файла `stream` больше не падает HTML-500
  - если локальный файл потерян, код очищает битую ссылку в БД и при наличии `recording_id` продолжает через Mango fallback
  - та же проверка добавлена для `transcribe`, чтобы не ловить следующий 500 на том же звонке
- Деплой этой правки уже сделан на prod и backend-контейнер поднят как `healthy`
...
**Известное ограничение по звонкам (было, теперь исправляется на сервере):** запись `CallLog` создаётся не сразу при звонке, а только когда отрабатывает
`sync_mango_calls`. Чтобы не зависеть от открытия раздела «Телефония» в CRM, добавлен отдельный фоновый worker
`python manage.py run_telephony_sync_worker --all-companies` и сервис `telephony-sync` в `docker-compose.prod.yml`.
Он должен работать постоянно на сервере и подтягивать звонки, даже если никто не открывает UI.
...
**Сканеры / legacy-домен**
- `sport-max.com.ru` / `www.sport-max.com.ru` → **301** на `https://schedule.sportmax.fit/`
- Блоклист IP: `deploy/nginx/scanner-blocked-ips.conf` (добавить IP строкой `1.2.3.4 1;` и `nginx -s reload`)
- Блоклист URI (wp-admin, .env, …): `deploy/nginx/scanner-blocked-uris.map`
- Хелпер: `deploy/nginx/block-scanner-ip.sh 1.2.3.4 "note"`
- Чужой Host / probe → nginx `444` (не доходит до Django); `DisallowedHost` больше не шлётся в Telegram
...
- Поиск клиентов и звонков теперь нормализует телефонные номера, чтобы находить записи с форматами `+7 (910) ...`, `8(...)` и чистыми цифрами
...
Логин: `admin` / `<demo-password>`, компания `sportmax`  
Локальный UI: `http://127.0.0.1:3000`

**Важно:** не запускать `npm run build` при работающем `npm run dev` (битый кэш `.next`).
Телефония больше не должна зависеть от открытия страницы: sync должен идти через `telephony-sync`.
